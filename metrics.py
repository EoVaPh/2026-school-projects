...
def get_window_rmsd(seq_1_aligned, seq_2_aligned, points_1, points_2, window_size):
    '''Get RMSD of alignment of residue pairs in window.'''
    ...
    for start in range(len(paired_indices) - window_size + 1):
        ...
        first_pair = frame_pairs[len(frame_pairs) // 10]#[0]
        last_pair = frame_pairs[-len(frame_pairs) // 10]#[-1]
...
def get_window_lddt(seq_1_aligned, seq_2_aligned, points_1, points_2, window_size):
    paired_indices = get_aligned_indices(seq_1_aligned, seq_2_aligned)
    ...
    for start in range(len(paired_indices) - window_size + 1):
        ...
        id1_obj = struc.AtomArray(len(frame_points_1))
        id1_obj.coord = np.array(frame_points_1, dtype=float)
        id1_obj.coord.reshape(len(frame_points_1), 3)

        id2_obj = struc.AtomArray(len(frame_points_2))
        id2_obj.coord = np.array(frame_points_2, dtype=float)
        id2_obj.coord.reshape(len(frame_points_2), 3)

        # 4. Fill required metadata annotations to trick internal filters (e.g., residue separation)
        # Setting sequence and chain data manually allows lddt() to execute flawlessly
        for obj in [id1_obj, id2_obj]:
            obj.res_id = np.arange(1, len(frame_points_1) + 1)
            obj.res_name = np.array(["GLY"] * len(frame_points_1))
            obj.atom_name = np.array(["CA"] * len(frame_points_1))
            obj.chain_id = np.array(["A"] * len(frame_points_1))
...
#bin_indices = np.digitize(all_mean_idr, bin_edges) - 1
#bin_indices = bin_indices.clip(0, num_bins - 1)   # keep within range

# 2. Sum and count per bin, then divide
#y_sums = np.bincount(bin_indices, weights=all_rmsd, minlength=num_bins)
#counts = np.bincount(bin_indices, minlength=num_bins)
# Avoid division by zero (empty bins → NaN by default)
#y_means = np.divide(y_sums, counts, out=np.full_like(y_sums, np.nan), where=counts>0)
