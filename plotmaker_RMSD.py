...
def plot_protein_families(data, output_image='RMSD_plot.png'):
    """
    Plots floating boxes (Q1-Q3) with range whiskers (min/max), median, mean, and data points for RMSD.
    """
    if not data:
    ...
    plt.scatter(x, medians, color='#e63946', marker='_', s=400, linewidths=3.5,
    ...
    # 5. Plot individual pairwise RMSD values with horizontal jitter
    for i, name in enumerate(fam_names):
        ...
        plt.scatter(x_points, y_points, color='#1d3557', alpha=0.4,
                    ...
                    label='Pairwise RMSD Values' if i == 0 else "")
    ...
    plt.ylabel('Structural Distance (RMSD, Å)')
    plt.title('Distribution of Pairwise Comparisons by Protein Families (Filter: ≥ 10 elements)')

    # Set the lower limit to 0.0, upper limit auto-scales to max value with a 10% padding
    plt.ylim(0.0, max(maxs) * 1.1 if maxs else 5.0)
...
if __name__ == '__main__':
    # Define your input data filename here (change to your actual RMSD file name)
    input_file = 'family_core_RMSD.txt'
